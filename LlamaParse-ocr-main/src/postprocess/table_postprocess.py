...
def remove_empty_trailing_columns(rows: list[list[str]]) -> list[list[str]]:
    ...
    def meaningful(row_idx: int, cell: str) -> bool:
        """Bỏ qua cell rỗng, separator và placeholder khi tính width thật."""
...
def is_probably_violation_table_data_row(row: list[str]) -> bool:
    """Nhận diện data row của bảng Phụ lục vi phạm/kỷ luật có cột TT.

    Bảng này có schema thật:
    TT | Nội dung vi phạm | Lần 1 | Lần 2 | Lần 3 | Ghi chú

    Một số ô xử lý trong PDF gốc được merge ngang qua cả 3 cột Lần 1-3.
    Khi chuyển sang Markdown, cần giữ/lặp nội dung ở cả 3 cột thay vì gôm về
    1 ô, nếu không hàng `01` chỉ còn nội dung ở cột Lần 1 và trống Lần 2-3.
    """

    if len(row) < 5:
        return False
    first = normalize_key(row[0])
    second = normalize_key(row[1]) if len(row) > 1 else ""
    return bool(re.fullmatch(r"\d{1,3}", first)) and bool(second)


def is_violation_discipline_table_header(cells: list[str]) -> bool:
    """Nhận diện header bảng `Nội dung vi phạm và khung xử lý kỷ luật`."""

    keys = [normalize_key(cell) for cell in cells]
    joined = " | ".join(keys)
    return (
        "tt" in keys[:2]
        and "noi dung vi pham" in joined
        and "hinh thuc xu ly" in joined
        and "ghi chu" in joined
        and ("lan 1" in joined or "lan1" in joined)
        and ("lan 2" in joined or "lan2" in joined)
        and ("lan 3" in joined or "lan3" in joined)
    )


def looks_like_merged_discipline_action(cell: str) -> bool:
    """Nhận diện nội dung xử lý vốn merge ngang qua các cột Lần 1-3."""
    ...
    merged_markers = (
        "khi vi pham",
        "tuy muc do",
        "de nghi nha truong xu ly",
        "co the de nghi nha truong xu ly",
        "giao cho co quan chuc nang",
    )
    return any(marker in key for marker in merged_markers)


def expand_merged_discipline_action_cells(rows: list[list[str]]) -> list[list[str]]:
    """Lặp nội dung cell merge ngang qua 3 cột cảnh báo/kỷ luật trong bảng vi phạm.

    Markdown pipe table không có colspan thật. Với bảng Phụ lục, các ô như hàng
    `01` trong PDF gốc chiếm đồng thời `Lần 1`, `Lần 2`, `Lần 3`. Nếu sau OCR
    chỉ còn nội dung ở `Lần 1` và hai cột sau rỗng, hàm này sao chép nội dung
    đó sang `Lần 2` và `Lần 3` để không mất nghĩa merge.
    """

    if not rows or not is_violation_discipline_table_header(rows[0]):
        return rows
    ...
    for row_idx, row in enumerate(normalized):
        ...
        lan2_empty = not lan2
        lan3_empty = not lan3
        lan2_is_footnote = bool(re.fullmatch(r"\(?\d+\)?", normalize_key(lan2).replace(" ", "")))

        if action and lan3_empty and looks_like_merged_discipline_action(action):
            merged_action = f"{action} {lan2}".strip() if lan2_is_footnote else action
            if lan2_empty or lan2_is_footnote:
                row[2] = merged_action
                row[3] = merged_action
                row[4] = merged_action
...
def compact_repeated_span_cells(row: list[str]) -> list[str]:
    ...
    Riêng data row của bảng vi phạm/kỷ luật được giữ nguyên vì các ô merge
    ngang qua `Lần 1`, `Lần 2`, `Lần 3` cần được lặp nội dung để mô phỏng
    colspan trong Markdown.
    ...
    if is_probably_violation_table_data_row(row):
        return row
...
def normalize_single_table(block: list[str]) -> list[str]:
    ...
    rows = expand_merged_discipline_action_cells(rows)
...
def parse_html_table_rows(section_html: str) -> list[list[str]]:
    ...
    for tr_match in TR_RE.finditer(section_html):
        ...
        def flush_pending_until(target_col: int | None = None) -> None:
            """Đổ các cell rowspan đang chờ vào row hiện tại đến cột đích."""
