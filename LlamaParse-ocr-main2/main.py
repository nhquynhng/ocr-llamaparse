...
def auto_output_dir_for(input_file: Path) -> Path | None:
    """Suy ra thư mục output tương ứng theo cấu trúc file PDF gốc.

    Dataset/02_Attachments/PDFs/<GROUP>/<Category>/file.pdf
        -> Dataset/06_Processing/01_OCR_Output/PDFs_<GROUP>/<Category>/

    Trả về None nếu file gốc không nằm trong .../PDFs/<GROUP>/... để caller
    fallback về thư mục output do người dùng chỉ định.
    """

    parts = input_file.resolve().parts
    lowered = [p.lower() for p in parts]
    if "pdfs" not in lowered:
        return None

    idx = lowered.index("pdfs")
    # Cần ít nhất <GROUP> ngay sau "PDFs".
    if idx + 1 >= len(parts):
        return None
    ...
    return OCR_OUTPUT_ROOT.joinpath(f"PDFs_{group}", *category_parts)
