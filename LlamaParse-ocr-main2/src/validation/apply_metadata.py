...
DATASET_ROOT = Path(r"D:\Code\CTU_Student_Service\Dataset")
...
def canonical_markdown_path(source_path: Path | None, md_path: Path) -> str:
    if source_path:
        relative = source_path.relative_to(DATASET_ROOT / "02_Attachments")

        # PDFs/PDT -> PDFs_PDT
        root_folder = f"{relative.parts[0]}_{relative.parts[1]}"

        # QuyTrinh/qt_hoc_2_CTDT.pdf
        rest = Path(*relative.parts[2:]).with_suffix(".md")
    ...
    metadata.update(
        {
            ...
            "canonical_markdown_path": first_non_empty(existing.get("canonical_markdown_path"),canonical_markdown_path(source_path, md_path),),
