...
def dump_front_matter(metadata: dict[str, Any]) -> str:
    ...
    for key in FIELD_ORDER:
        ...
        if key == "responsible_department":
            # Luôn dump dạng list (kể cả rỗng)
...
#     """Trả về đường dẫn tương đối (posix) so với thư mục chạy script.

#     PROJECT_ROOT ở đây chính là thư mục LlamaParse-ocr-main2, nên các file nằm
#     trong Dataset (ngoài thư mục này) sẽ ra dạng ../Dataset/...
#     """
...
#     except ValueError:
#         # Khác ổ đĩa trên Windows -> không tạo được đường dẫn tương đối.
...
def path_for_metadata(path: Path) -> str:
    ...
    if "dataset" in lowered:
        idx = lowered.index("dataset")
        return "../" + "/".join(parts[idx:])

    return resolved.as_posix()

def canonical_markdown_path(source_path: Path | None, md_path: Path) -> str:
    """Trả về đúng đường dẫn canonical của file Markdown sẽ được ghi ra.

    Trước đây hàm này tự suy luận lại từ source_path bằng DATASET_ROOT
    hard-code, nên DOCX bị ép sang DOCX_CTSV và tên file cũng có thể lệch
    với output thật. Ở bước OCR, main.py đã quyết định output_path chính xác,
    vì vậy metadata phải bám theo md_path thay vì tính lại lần nữa.
    """

    return path_for_metadata(md_path)
...
def build_metadata(
    ...
    # responsible_department: giữ nguyên list nếu đã có, không tự suy ra
    existing_dept = existing.get("responsible_department")
    responsible_department: list[Any] = list_value(existing_dept) if not is_empty(existing_dept) else []
    ...
    metadata.update(
        {
            ...
            "issuing_authority": string_value(existing.get("issuing_authority") or "Đại học Cần Thơ"),
            "responsible_department": responsible_department,
            "signer_name": string_value(existing.get("signer_name") or "HT Hà Thanh Toàn"),
            ...
            "review_status": normalize_enum(existing.get("review_status"), BACKEND_REVIEW_STATUSES, "approved"),
            ...
            "canonical_markdown_path": (
                canonical_markdown_path(source_path, md_path)
                if args.overwrite_auto
                else first_non_empty(existing.get("canonical_markdown_path"), canonical_markdown_path(source_path, md_path))
            ),
